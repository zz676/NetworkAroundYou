...
package edu.nyu.ga2620.network.utils;
...
import edu.nyu.ga2620.network.ActivityMain;
import edu.nyu.ga2620.network.R;
...
public class Prefs extends PreferenceActivity implements
	...
	private static final String URL_EMAIL = "zhisheng.zhou@nyu.edu";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		//Support API Level 10 or lower,
		addPreferencesFromResource(R.xml.preferences);
		ctxt = getApplicationContext();
		
		//Support API's below 11
		...
		contact.setOnPreferenceClickListener(new OnPreferenceClickListener() {
			public boolean onPreferenceClick(Preference preference) {
				...
				emailIntent.putExtra(Intent.EXTRA_EMAIL,
						new String[] { URL_EMAIL });
				emailIntent.putExtra(Intent.EXTRA_SUBJECT, "Network Around You");
				try {
		...
		try {
			version.setSummary(getPackageManager().getPackageInfo(
					ActivityMain.PKG, 0).versionName);
		} catch (NameNotFoundException e) {
			version.setSummary("1.x");
		}
	...
	public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
		if (key.equals(KEY_PORT_START) || key.equals(KEY_PORT_END)) {
			...
			CheckBoxPreference cb = (CheckBoxPreference) ps
					.findPreference(KEY_CIDR_CUSTOM);
			if (cb.isChecked()) {
				((CheckBoxPreference) ps.findPreference(KEY_IP_CUSTOM))
						.setChecked(false);
			}
			sendBroadcast(new Intent(WifiManager.WIFI_STATE_CHANGED_ACTION));
		} else if (key.equals(KEY_IP_CUSTOM)) {
			CheckBoxPreference cb = (CheckBoxPreference) ps
					.findPreference(KEY_IP_CUSTOM);
			if (cb.isChecked()) {
				((CheckBoxPreference) ps.findPreference(KEY_CIDR_CUSTOM))
						.setChecked(false);
			}
			sendBroadcast(new Intent(WifiManager.WIFI_STATE_CHANGED_ACTION));
		}
	...
	private void checkTimeout(String key_pref, String key_cb, boolean value) {
		EditTextPreference timeout = (EditTextPreference) ps
				.findPreference(key_pref);
	...
	private void checkIpRange() {
		EditTextPreference ipStartEdit = (EditTextPreference) ps
				.findPreference(KEY_IP_START);
		EditTextPreference ipEndEdit = (EditTextPreference) ps
				.findPreference(KEY_IP_END);
		...
		if (!matcher1.matches() || !matcher2.matches()) {
			...
			Toast.makeText(ctxt, R.string.preferences_error4, Toast.LENGTH_LONG)
					.show();
			return;
		}
		...
		try {
			...
			if (ipStart > ipEnd) {
				ipStartEdit.setText(before_ip_start);
				ipEndEdit.setText(before_ip_end);
				Toast.makeText(ctxt, R.string.preferences_error1,
						Toast.LENGTH_LONG).show();
			}
		} catch (NumberFormatException e) {
			ipStartEdit.setText(before_ip_start);
			ipEndEdit.setText(before_ip_end);
			Toast.makeText(ctxt, e.getLocalizedMessage(), Toast.LENGTH_LONG)
					.show();
		}
	}
	...
	private void checkPortRange() {
		...
		EditTextPreference portStartEdit = (EditTextPreference) ps
				.findPreference(KEY_PORT_START);
		EditTextPreference portEndEdit = (EditTextPreference) ps
	...
	// EditTextPreference threads = (EditTextPreference)
	// ps.findPreference(KEY_NTHREADS);
	// int nthreads = 0;
	// try {
	...
	// threads.setText(DEFAULT_NTHREADS);
	// Toast.makeText(ctxt, R.string.preferences_error2,
	// Toast.LENGTH_LONG).show();
	// }
	// }
}
