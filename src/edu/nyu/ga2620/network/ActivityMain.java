...
import edu.nyu.ga2620.network.utils.DbUpdate;
...
final public class ActivityMain extends Activity {
	...
	public static final String PKG = "edu.nyu.ga2620.network";
	...
	private void phase3(final Context ctxt) {
		...
		try {
			if (prefs.getInt(Prefs.KEY_RESET_SERVICESDB,
					Prefs.DEFAULT_RESET_SERVICESDB) != getPackageManager()
					.getPackageInfo(PKG, 0).versionCode) {
		...
		protected void onPreExecute() {
			...
			if (d != null) {
				try {
					...
					progress = ProgressDialog.show(d, "",
							d.getString(R.string.task_services));
				} catch (Exception e) {
					if (e != null) {
		...
		protected Void doInBackground(Void... params) {
			...
			if (d != null) {
				...
				try {
					...
					values.put("mac", net.macAddress.replace(":", "")
							.toUpperCase());
					values.put("name",
							d.getString(R.string.discover_myphone_name));
		...
		protected void onPostExecute(Void unused) {
			...
			if (d != null) {
				...
				try {
					Editor edit = prefs.edit();
					edit.putInt(
							Prefs.KEY_RESET_SERVICESDB,
							d.getPackageManager().getPackageInfo(PKG, 0).versionCode);
					edit.commit();
				} catch (NameNotFoundException e) {
					Log.e(TAG, e.getMessage());
				} finally {
